...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Atom " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Atom Core is probably already running."), addrBind.ToString());
        else
...
void RelayTransaction(const CTransaction& tx, const uint256& hash, const CDataStream& ss)
{
    ...
    CInv inv(MSG_TX, hash);
    {
        ...
        {
        	LogPrintf("RelayTransaction2   &&   while---------------\n");
    ...
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
    	LogPrintf("RelayTransaction2   &&   BOOST_FOREACH---------------\n");
        if(!pnode->fRelayTxes)
            continue;
        LOCK(pnode->cs_filter);
        if (pnode->pfilter)
        {
        	LogPrintf("RelayTransaction2   &&   BOOST_FOREACH && pnode if---------------\n");
            if (pnode->pfilter->IsRelevantAndUpdate(tx, hash))
                pnode->PushInventory(inv);
        } else
        	LogPrintf("RelayTransaction2   &&   BOOST_FOREACH && pnode else---------------\n");
            pnode->PushInventory(inv);
    }
}
