...
// Copyright (c) 2009-2015 The Atom developers
...
#if defined(NDEBUG)
# error "Atom cannot be compiled without assertions."
#endif
...
{
	LogPrintf("AcceptBlock-----------------aaaaaaaaaaaa\n");
    AssertLockHeld(cs_main);
    // Check for duplicate
    uint256 hash = block.GetHash();
    if (mapBlockIndex.count(hash))
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        // Check proof of work
        if (nHeight > 26949 && block.nBits != GetNextWorkRequired(pindexPrev, &block))
	...
	if (block.GetBlockTime() <= pindexPrev->GetBlockTime() - 2 * 12 * 60)
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    for (unsigned int i = 0; i < vWorkQueue.size(); i++)
    {
             ...
             ++mi)
        {
            ...
            CValidationState stateDummy;
            LogPrintf("AcceptBlock(block, stateDummy)");
